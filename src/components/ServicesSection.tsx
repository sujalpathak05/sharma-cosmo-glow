...
const ServicesSection = () => {
  ...
  return (
    <section id="services" ref={sectionRef} className="py-24 lg:py-32 section-padding overflow-hidden">
      <div className="section-container">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((s, i) => (
            <div
              key={s.title}
              className={`group bg-card rounded-2xl p-7 border border-border card-hover cursor-pointer transition-all duration-700 ${
                visible ? "opacity-100 translate-y-0 scale-100" : "opacity-0 translate-y-8 scale-95"
              }`}
              style={{ transitionDelay: `${0.1 + i * 0.1}s` }}
              onClick={() => setExpandedIdx(expandedIdx === i ? null : i)}
            >
              <div className="w-12 h-12 rounded-xl bg-rose-soft flex items-center justify-center mb-5 group-hover:bg-primary group-hover:text-primary-foreground transition-all duration-300 group-hover:scale-110 group-hover:rotate-3">
              ...
              {expandedIdx === i && (
                <p className="text-body text-sm mt-3 pt-3 border-t border-border animate-fade-up">{s.detail}</p>
              )}
