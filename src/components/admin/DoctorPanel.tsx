...
const DoctorPanel = ({ appointments, onGenerateBill }: DoctorPanelProps) => {
  ...
  if (view === "followup") filtered = filtered.filter((item) => item.status === "completed" || item.service.toLowerCase().includes("follow"));
  if (search.trim()) {
  ...
  const topService = Object.entries(appointments.reduce<Record<string, number>>((acc, item) => {
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          ...
          <div className="grid gap-3 sm:grid-cols-3 xl:w-[540px]">
            ...
            <div className="rounded-[22px] bg-white/80 px-4 py-4 shadow-sm">
              ...
              <p className="mt-2 font-display text-3xl text-foreground">{appointments.filter((item) => item.status === "completed").length}</p>
              <p className="text-sm text-muted-foreground">patients ready for recall</p>
            </div>
            <div className="rounded-[22px] bg-white/80 px-4 py-4 shadow-sm">
      ...
      <section className="glass-panel rounded-[30px] border border-[#f0ddb8] p-5 shadow-[0_24px_60px_-42px_rgba(79,51,8,0.34)] sm:p-6">
            ...
            {([
              ...
              <button
                ...
                className={cn(
                  ...
                  view === key ? "bg-primary text-primary-foreground" : "bg-white/70 text-muted-foreground hover:text-foreground",
                )}
              >
                {label}
              </button>
        ...
        <div className="mt-5 overflow-hidden rounded-[24px] border border-[#eadfc8] bg-white/70">
          <div className="grid grid-cols-[0.45fr,0.9fr,1fr,0.9fr,0.8fr,0.8fr,0.7fr,1fr] gap-4 border-b border-[#eadfc8] px-4 py-3 text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">
            <span>No.</span>
          ...
          {filtered.length === 0 ? (
            ...
            filtered.slice(0, 12).map((item, index) => {
              ...
              return (
                <div key={item.id} className="grid grid-cols-[0.45fr,0.9fr,1fr,0.9fr,0.8fr,0.8fr,0.7fr,1fr] gap-4 border-b border-[#f3ead8] px-4 py-4 text-sm last:border-b-0">
                  <span>{index + 1}</span>
                  ...
                  <span>{view === "followup" ? "Follow-up" : item.service}</span>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button className="rounded-full bg-[#5a49d6] px-3 py-1.5 text-xs font-medium text-white">Consult</button>
                    <button onClick={() => onGenerateBill(item)} className={cn("rounded-full px-3 py-1.5 text-xs font-medium", isBilled ? "bg-emerald-100 text-emerald-800" : "bg-white border border-[#d4c7ff] text-[#5a49d6]")}>
