...
const PharmacyPanel = ({ appointments }: PharmacyPanelProps) => {
  ...
  const saleSubtotal = salePreviewItems.reduce((sum, item) => sum + item.total, 0);
  const saleDiscountPercent = clampDiscountPercent(saleForm.discountPercent);
  const saleDiscountAmount = Math.round((saleSubtotal * saleDiscountPercent) / 100);
  const saleDraftTotal = Math.max(0, saleSubtotal - saleDiscountAmount);
