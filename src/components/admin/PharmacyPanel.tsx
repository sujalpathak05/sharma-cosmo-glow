            ...
            <label className="text-sm font-medium text-foreground">
              ...
              <select
                ...
                onChange={(event) => {
                  ...
                  updateRows(type, row.key, { medicineId: event.target.value, ...(type === "purchase" ? { price: medicine ? String(medicine.unitPrice) : row.price } : {}) });
                }}
                className="mt-2 w-full rounded-xl border border-border bg-background px-4 py-3 text-sm outline-none"
              >
                <option value="">Select medicine</option>
                {data.pharmacyMedicines.map((item) => <option key={item.id} value={item.id}>{item.name} ({item.stock} in stock)</option>)}
              </select>
            </label>
            <label className="text-sm font-medium text-foreground">
              Qty
    ...
    <div className="space-y-6">
      ...
      {view === "masters" ? (
        <Surface>
          ...
          <div className="mt-5 overflow-hidden rounded-[24px] border border-[#eadfc8] bg-white/70"><div className="grid grid-cols-[1.1fr,0.9fr,0.8fr,0.8fr,0.8fr] gap-4 border-b border-[#eadfc8] px-4 py-3 text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground"><span>Medicine</span><span>Generic</span><span>Group</span><span>MFG</span><span>Batch</span></div>{filteredMedicines.map((item) => <div key={item.id} className="grid grid-cols-[1.1fr,0.9fr,0.8fr,0.8fr,0.8fr] gap-4 border-b border-[#f3ead8] px-4 py-4 text-sm last:border-b-0"><span className="font-medium text-foreground">{item.name}</span><span>{item.generic}</span><span>{item.group}</span><span>{item.manufacturer}</span><span>{item.batch}</span></div>)}</div>
        </Surface>
      ) : null}
